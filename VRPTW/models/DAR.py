...
class DAR(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        # 计算节点坐标差异
        depot_xy_expanded = depot_xy.expand_as(node_xy)
        coord_diff = node_xy - depot_xy_expanded
        node_features = torch.cat((node_xy, coord_diff, node_demand[:, :, None], node_tw_start[:, :, None], node_tw_end[:, :, None]), dim=2)
    ...
    def forward(self, state, cur_dist,selected=None):
        ...
        if state.selected_count == 0:
            ...
            attr = self._get_attr(state)
            probs = self.decoder(encoded_last_node, attr, cur_dist,ninf_mask=state.ninf_mask)
            if selected is None:
                selected, prob = self._select_action(probs, state)
            else:
                selected = selected
        ...
        while True:
            if self.training or self.eval_type == 'softmax':
                try:
                    ...
                    print(f">> Catch Exception: {exception}, on the instances of {state.PROBLEM}")
                    exit(0)
            else:
...
class SINGLE_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.embedding_node = nn.Linear(7, embedding_dim)  # Including coord_diff
...
class SINGLE_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, attr,cur_dist, ninf_mask):
